    ...
    for(int i = 1 ; i <= 16 ; i++)
        for(int j = 1 ; j <= 16 ; j++){
            ...
            connect(M , &MineBlock::FindOthers , this ,[=](){       //连接Findothers信号，翻开该方格周围的点
                for(int k = 0 ; k < 9 ; k++){
                    if(this->Checkinmap(i+dx[k],j+dy[k] ,16 , 16) == false){  //在地图外就continue
                        continue;
                    }
            ...
            connect(M , &MineBlock::S_Findthis , this,[=](){            //连接被翻开信号
                if(this->RestUnMineNum == 16 * 16 - 40){            //如果是第一次点击
                    t->start(1000);
                    srand((unsigned)time(NULL));        //生成随机数
                    int Random_Mine[257];        //随机分配雷
                    memset(Random_Mine , 0 , sizeof(Random_Mine));
                    for(int i = 1 ; i <= 40 ; i++)
                        Random_Mine[i] = -1;        //-1为雷
                    random_shuffle(Random_Mine + 1 , Random_Mine + 256);     //打乱
                    ...
                    for(int i = 1 ; i <= 16 ; i++){
                        for(int j = 1 ; j <= 16 ; j++){
                            this->CountRoundMine(i , j , 16 , 16);            //统计每个格子旁边有几颗雷
                        }
                    }
                }
